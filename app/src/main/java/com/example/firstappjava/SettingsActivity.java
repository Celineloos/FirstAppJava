        ...
        private String updateSummary(Preference p) {
            ...
            else if (p instanceof MultiSelectListPreference)
            {
                ...
                for (String value : values)
                {
                    ...
                    if (index >= 0)
                    {
                        p.setSummary(p.getSummary() + (p.getSummary().length() == 0 ? "" : ", ") + ((MultiSelectListPreference) p).getEntries()[index]);
                    }
                }
            }
            ...
            else if (p instanceof PreferenceGroup)
            {
                ...
                for (int i = 0; i < preference.getPreferenceCount(); i++)
                {
                    p.setSummary(p.getSummary() + (p.getSummary().length() == 0 ? "" : ", ") + updateSummary(preference.getPreference(i)));
                }
            }
